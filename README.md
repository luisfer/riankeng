...
- `/learn/` is the course. Hash routes live under it, for example `/learn/#/account`. On Vercel it sits behind the `rk_gate` cookie set by the password. Log out on the Account page returns to `/`.
...
Locally the gate is open: with `SITE_PASSWORD` empty, any password passes and `/api/session` answers `in: true`. Set it in `.env.local` to try the real flow.
...
Romanization follows the textbook system in `content/system.ts` (à â á ǎ, g/bp/dt, ε ɔ ə ụ). The open e is stored as Greek ε.
