...
describe('decideGate', () => {
  ...
  it('keeps the demo clips public and the rest of the audio gated', async () => {
    ...
    expect(await decideGate(clipUrl('w:glai'), null, SECRET, true)).toEqual({ kind: 'redirect', to: '/?signin' })
    expect(await decideGate(clipUrl('w:glai'), await cookieFor(SECRET), SECRET, true)).toEqual({ kind: 'shell' })
  })
  ...
  it('names an unset password on Vercel, and stays open locally', async () => {
    ...
    expect(await decideGate('/audio/w:glai.mp3', null, undefined, false)).toEqual({ kind: 'shell' })
  })
})
